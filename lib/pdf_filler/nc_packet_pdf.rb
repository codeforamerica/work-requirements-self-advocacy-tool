module PdfFiller
  class NcPacketPdf < PacketPdf
    ...
    # pdf_fields (base class) doesn't call hash_for_fillable_pdf/hash_for_generated_pdf above, so
    # the homeschool fields need their own override here too -- @nc_screener only exists for NC
    # screeners, so nothing else would supply these for the new HTML path otherwise.
    def pdf_fields
      super.merge(
        homeschool_hours: @nc_screener.homeschool_hours.to_s,
        homeschool_name: @nc_screener.homeschool_name,
