...
RSpec.describe Screener, type: :model do
  ...
  describe "validations" do
    ...
    context "with_context :feedback_result" do
      ...
      it "must not have unsubmitted_write_in value longer than FeedbackResultController::CHARACTER_LIMIT, if a value is set" do
        screener = build(:screener, unsubmitted_write_in: "This is just a test value.")
        # Valid value that is not too long
        expect(screener.valid?(:feedback_result)).to eq true
        ...
        limit = FeedbackResultController::CHARACTER_LIMIT
        text = SecureRandom.alphanumeric(limit + 1)
        screener.assign_attributes(unsubmitted_write_in: text)

        screener.valid?(:feedback_result)
        expect(screener.errors[:unsubmitted_write_in]).to be_present
      end
    end
